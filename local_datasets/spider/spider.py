...
import datasets
from local_datasets import utils, dataset
...
"""
import os
def dump_db_json_schema(db_path: str, db_id: str):
    schema_path = db_path.split(f"{db_id}.sqlite")[0] + "schema.sql"
    with open(os.join.path(schema_path), 'r') as fp:
        create_table_str = fp.read()
        return create_table_str
"""



class Spider(dataset.DataSet):
