...
from third_party.test_suite import evaluation as sparc_evaluation
...
def compute_exact_match_metric(predictions, references) -> Dict[str, Any]:
    ...
    for reference in references:
        if reference["db_id"] not in foreign_key_maps:
            foreign_key_maps[reference["db_id"]] = sparc_evaluation.build_foreign_key_map(
                {
    ...
    evaluator = sparc_evaluation.Evaluator(
